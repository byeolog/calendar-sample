...
export default class ToastTable extends Component {
  ...
  selectedView = this.state.view || this.props.view;
  ...
  onBeforeDeleteSchedule(res) {
    ...
    const idx = this.state.scheduleList.findIndex(
      item => item.id === res.schedule.id
    );
    this.setState({
      scheduleList: [...this.state.scheduleList.splice(idx, 1)]
    });
  }
    ...
    return (
      <div>
        ...
        <Calendar
          ...
          onClickTimezonesCollapseBtn={this.onClickTimezonesCollapseBtn.bind(
            this
          )}
        />
      </div>
